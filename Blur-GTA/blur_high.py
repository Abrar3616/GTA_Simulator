...
while(True):
    im1 = cv.imread(dir + std::to_string(i) + ".jpg")
    if im1 is None:
        sleep(1)
        if im1 is None:
            break
   
    im2 = cv.imread(dir + std::to_string(i-1) + ".jpg")
    im3 = cv.imread(dir + std::to_string(i-2) + ".jpg")
    im4 = cv.imread(dir + std::to_string(i-3) + ".jpg")
    ...
    cv.imwrite("/home/abrar/gta_blur/" + std::to_string(i) + ".jpg", arr)
    i+=1
